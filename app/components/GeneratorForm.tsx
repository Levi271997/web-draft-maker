 ...
 * The answers are snapshotted here rather than read live: retouching the
 * pickers after a result shouldn't silently change what "Friendlier"
 * regenerates.
 */
type LastRequest = {
  url: string;
...
export default function GeneratorForm({ bookingUrl }: { bookingUrl: string }) {
  ...
  async function run(
    ...
    setPending({ subject: request.url, area: request.facts.area });
    setLoading(true);
    ...
    <div className="flex flex-col gap-8">
      <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200 sm:p-8">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (url.trim()) run({ url, sections, goals, facts });
          }}
          noValidate
        >
          <label
          ...
          <p className="mb-4 text-sm text-ink-soft">
            Enter the homepage you want us to analyse. We&apos;ll read whatever
            is publicly visible.
          </p>
      ...
      {loading && !result && (
        <LoadingState subject={pending?.subject ?? ""} area={pending?.area} />
      )}
